...
namespace CoAP.Examples
{
    class CoAPClient
    {
        ...
        public static void Mains(String[] args, string info,string uname,string IP)
        {
                ...
                if (byEvent)
                {
                    request.Responded += delegate(Object sender, ResponseEventArgs e)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                case "voltage":
                                    voltage = response.PayloadString;
                                    break;
                                case "temperature":
                                    temperature = response.PayloadString;
                                    break;
                                case "light":
                                    light = response.PayloadString;
                                    InsertInfo(IP, uname);
                                    break;
                                default:
                                    break;
        ...
        public static void InsertInfo(string IP,string uname)
        {
            InfoData infodata = new InfoData();

            infodata.username = uname;
            infodata.light = light;
            infodata.temperature = temperature;
            infodata.voltage = voltage;
            infodata.time = DateTime.Now.ToString();
            infodata.IP = IP;

            if (InfoDataOperation.insertInfo(infodata))
            {
                MessageBox.Show("获取成功！", "提示");
            }
            else
            {
                MessageBox.Show("获取失败！", "错误");
            }
        }
